...
- Keep semantic layout and text in HTML/CSS, but do not make essential first-viewport artwork depend on empty positioned elements whose only content is a CSS shape. A 2026-08-30 real-draft preview retained the background and text while losing such moon and water-line decorations. Render a critical CSS-designed scene to a raster asset, upload it in `article` mode, and use that HTTPS image in the body while the remaining HTML/CSS, SVG/SMIL, and media continue to serve their own roles.
