...
Treat semantic HTML, inline CSS, SVG/SMIL motion, raster imagery, and other compatible media as one coordinated design system, not mutually exclusive modes. Use them together by default when they have complementary roles in meaning, navigation, atmosphere, evidence, or pacing. Omit a medium only when it has no useful role or the sanitized WeChat draft proves it incompatible; never apply a blanket ban merely for convenience. Motion and advanced effects must degrade to a coherent static reading experience.

Do not use or reconstruct a layout template, preset composition, predefined theme, house style, component recipe, design token set, example article, or default typography and spacing system. Do not impose a fixed creative workflow. Infer the visual strategy from the specific content, audience, and context. Use the strongest current techniques and contemporary aesthetic judgment that survive the verified platform boundary; create the hierarchy, composition, pacing, type treatment, color, media, and responsive behavior independently. Every design may take a different form.
