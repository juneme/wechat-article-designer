...
- **Hard-edge dopamine recruitment article**: read [`references/dopamine-editorial-recruitment.md`](references/dopamine-editorial-recruitment.md), then start from [`assets/dopamine-editorial-recruitment-article.html`](assets/dopamine-editorial-recruitment-article.html). Keep the final WeChat fragment single-column and use text-owned padding inside image placeholders so inserted images touch the inner frame.
...
- If a pasted photo appears with huge empty margins, the image was inserted inside a padded placeholder; reduce placeholder padding and tell the user to delete the placeholder text before inserting the photo.
...
- If the frame disappears, the user copied only the image instead of the HTML frame; use an HTML placeholder frame or bake the frame into the image depending on the desired workflow.

## Photo Frame Pattern (Rounded Premium Variant)

This rounded version is verified in the WeChat editor and works well for warm, service-oriented, or lifestyle articles. It is not the universal default for every visual register.
...
- Single-line placeholder text, no emoji, no sub-caption inside the frame.
...
Do not place a large padded block around the future image. The white frame should provide the only persistent padding.
...
- Do not create photo space with huge top/bottom padding. Use the placeholder frame selected for the article’s visual register and a short delete-this-line instruction instead.
...
8. **Flex and overflow** - compact flex rows do not squeeze text at 320px; swipe strips use intentional `overflow-x:auto`, stable item counts, and no autoplay claim.
